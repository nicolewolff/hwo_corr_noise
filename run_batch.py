...
def calc_GP_Amplitude(snr_corr):
    amp = .279 * 10 / snr_corr  ### .279 and 10 are a pre-calculated amplitude/SNR pair for SNR calc at 0.55um
    return amp
...
for iteration in range (1,10):
    ...
    for i, line in enumerate(lines):
        if line.startswith('snr0'):
            ...
            lines[i] = "dirout  = outputs_correlated_new/snr_uncorr_{:.2f}".format(snr)+"_snr_tot_{:.2f}".format(snr_tot)+"_length_"+length+"/"+str(iteration)+"   \n"
    ...
    os.system("sh run.sh")
    # os.system("python3 rfast_analyze.py rfast_inputs.scr")
