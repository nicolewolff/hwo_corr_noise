...
#   intermediate quantities for computing layer radiative properties
    ...
    d     = 1 - np.multiply(np.exp(-2*np.multiply(np.multiply(a[ic],b[ic]),dtau[ic])),np.multiply(Ainf[ic],Ainf[ic])) # another convenient definition
    r[ic] = np.divide(np.multiply(Ainf[ic],(1 - np.exp(-2*np.multiply(np.multiply(a[ic],b[ic]),dtau[ic])))),d)
    t[ic] = np.divide(np.multiply(1 - np.multiply(Ainf[ic],Ainf[ic]),np.exp(-np.multiply(np.multiply(a[ic],b[ic]),dtau[ic]))),d)
...
#   intermediate quantities for computing layer radiative properties
    ...
    d     = 1 - np.multiply(np.exp(-2*np.multiply(np.multiply(f[ic],b[ic]),dtau[ic])),np.multiply(Ainf[ic],Ainf[ic])) # another convenient definition
    r[ic] = np.divide(np.multiply(Ainf[ic],(1 - np.exp(-2*np.multiply(np.multiply(f[ic],b[ic]),dtau[ic])))),d)
    t[ic] = np.divide(np.multiply(1 - np.multiply(Ainf[ic],Ainf[ic]),np.exp(-np.multiply(np.multiply(f[ic],b[ic]),dtau[ic]))),d)
...
# pixel geometry-insensitive cloud scattering terms
  if cld:
    ...
    if (phfc == 1): # improved two-term henyey-greenstein; zhang & li (2016) JQSRT 184:40
      ...
      fac = np.multiply(gc1,gc2)-gc3
      w   = np.multiply(fac,fac) - np.multiply(4*(gc2-np.multiply(gc1,gc1)),np.multiply(gc1,gc3)-np.multiply(gc2,gc2))
      iw0 = np.where(w < 0)
      if (iw0[0].size != 0):
        w[iw0] = 1.e-5
      de  = 2*(gc2-np.multiply(gc1,gc1))
      g1  = np.divide(gc3 - np.multiply(gc1,gc2) + np.sqrt(w),de) # eqn 7a
      g2  = np.divide(gc3 - np.multiply(gc1,gc2) - np.sqrt(w),de) # eqn 7b
      al  = 0.5*id + np.divide(3*np.multiply(gc1,gc2)-2*np.power(gc1,3)-gc3,2*np.sqrt(w)) # eqn 7c
...
# single scattering direct beam phase function
  p1 = pray(cosTh)*fray
  if cld:
    p1 = p1 + np.multiply((id-fray),pc)
...
#      mu0 = 2./3. # if ntg=1, setting mu0=mu1=2/3 approx. disk-average
#      mu1 = 2./3.
...
#     direct beam treatment
      Idir0 = F0*np.exp(-tau/mu0)  # downwelling direct solar intensity at each level
      I11   = np.zeros([Nlam,Nlev])
      if (mu0 == 0.5):
        I11[:,:-1] = np.multiply(np.multiply(Idir0[:,:-1],p1),np.multiply(dtau/mu0,np.exp(-dtau/mu0)))/2
      else:
        I11[:,:-1] = np.multiply(np.multiply(Idir0[:,:-1],p1),mu0/(mu0-0.5)*(np.exp(-dtau/mu0)-np.exp(-dtau/0.5)))/2
      I11[:,:-1] = np.multiply(I11[:,:-1],omega)/(4*np.pi) # upwelling single-scattered intensity at top of each layer
      I11[:,-1]  = As*mu0*F0*np.exp(-tau[:,-1]/mu0)/np.pi
      I1   = np.multiply(I11,np.exp(-tau/mu1)) # upwelling single-scattered intensity attenuated to TOA
      Idir = np.sum(I1,axis=1) # attenuated upwelling single-scattered intensity integrated over layers

#     layer scattered flux differential for direct down and up beams
      dF0  = np.multiply(omega,Idir0[:,:-1] - Idir0[:,1:])*mu0
      dF1  = np.multiply(omega,I11[:,1:] - I11[:,:-1])*mu1

#     down- and up-scattered radiation from direct downwelling beam
      pd0 = fray/2
      if cld:
        if (phfc == 0): # single-term henyey-greenstein
        ...
        pd0 = pd0 + np.multiply(fdc,id-fray)
      pu0 = id - pd0

#     down- and up-scattered radiation from single-scattered upwelling beam
      pu1 = fray/2
      if cld:
        if (phfc == 0): # single-term henyey-greenstein
          fuc = (pHG_int(gc1,(1-mu1**2)**0.5) + pHG_int(gc1,-(1-mu1**2)**0.5))/4
        if (phfc == 1): # improved two-term henyey-greenstein
          fr   = (pHG_int(g1,(1-mu1**2)**0.5) + pHG_int(g1,-(1-mu1**2)**0.5))/4
          bk   = (pHG_int(g2,(1-mu1**2)**0.5) + pHG_int(g2,-(1-mu1**2)**0.5))/4
          fuc  = np.multiply(al,fr) + np.multiply(id-al,bk)
        pu1 = pu1 + np.multiply(fuc,id-fray)
      pd1 = id - pu1

#     layer source terms; upwelling source removes single-scattered flux bookkept in direct beam
      su = np.multiply(dF0,pu0) + np.multiply(dF1,pu1) - I11[:,:-1]*mu1
      sd = np.multiply(dF0,pd0) + np.multiply(dF1,pd1)
...
#     lower boundary condition
      Ru[:,Nlev-1] = As
      Su[:,Nlev-1] = 0. # zero as direct beam treatment includes surface reflection
...
#        mb   - molar mass of background gas (g/mole)
...
#    dtauc0   - cloud extinction differential optical depth profile at lamc0 (Nlay)
#     lamc0   - wavelength where dtauc0 is referenced to (um)
...
def gen_spec(Nlev,Rp,a,As,em,p,t,t0,m,z,grav,Ts,Rs,ray,ray0,rayb,f,fb,
             mb,mmw0,mmr,ref,nu,alpha,threeD,
             gasid,ncia,ciaid,species_l,species_c,
             cld,sct,phfc,fc,g0,w0,Qc,dtauc0,lamc0,
  ...
  if ( np.any( pf == -1) and np.any( tf == -1) ): # varying p/t case
    sigma = np.exp(sigma_interp(np.log10(pm),1/tm))
  elif ( np.any( pf != -1) and np.any( tf == -1) ): # fixed p case
    sigma = np.exp(sigma_interp(1/tm))
  elif ( np.any( pf == -1) and np.any( tf != -1) ): # fixed t case
    sigma = np.exp(sigma_interp(np.log10(pm)))
  else: # fixed p and t
    sigma0 = np.exp(sigma_interp())
  ...
  if ray:
    if mmr:
      sigma_ray = rayleigh(lam,ray0/(mmw0/1.e3/Na),fm,fbm,rayb*mmw0[0]/mb) # m**2/kg
    else:
  ...
  # call radiative transfer model, only if the cloudless fraction is > 1 ppm
  if (not cld or 1-fc > 1e-6):
  ...
  if cld:
    ...
    # store gas opacities from cloud-free case
    dtaug      = np.copy(dtau)

    # cloudy optical properties
    q_interp   = interpolate.interp1d(lam,Qc,assume_sorted=True,fill_value="extrapolate")
    dtau_cld   = np.outer(Qc/q_interp(lamc0),dtauc0) # [Nlam x Nlay]
...
def inputs(filename_scr):
    ...
    for line in f:
      ...
      if (vn.lower() == 'fnr' ):
        ...
        if (len(vv[2]) > 0):
          ...
          while (len(vv[2]) > 0):
            vv = vv[2].partition(',')
            tpars = tpars,float(vv[0])
          tpars = list(collapse(tpars))
        else:
          tpars    = np.zeros(1)
          if (vv[0] != ''): tpars[:] = float(vv[0])
      elif (vn.lower() == 'rp'):
        Rp   = float(vv)
      elif (vn.lower() == 'mp'):
        Mp   = float(vv)
        ...
        if (len(vv[2]) > 0):
          ...
          while (len(vv[2]) > 0):
            vv = vv[2].partition(',')
            Apars = Apars,float(vv[0])
          Apars = list(collapse(Apars))
        else:
          Apars    = np.zeros(1)
          if (vv[0] != ''): Apars[:] = float(vv[0])
      elif (vn.lower() == 'em'):
        em   = float(vv)
      elif (vn.lower() == 'phfc'):
        phfc = int(vv)
        ...
        if (len(vv[2]) > 0):
          ...
          while (len(vv[2]) > 0):
            vv = vv[2].partition(',')
            opars = opars,float(vv[0])
          opars = list(collapse(opars))
        else:
          opars    = np.zeros(1)
          if (vv[0] != ''): opars[:] = float(vv[0])
      elif (vn.lower() == 'cpars'):
        vv = vv.partition(',')
        if (len(vv[2]) > 0):
          cpars = float(vv[0])
          while (len(vv[2]) > 0):
            vv = vv[2].partition(',')
            cpars = cpars,float(vv[0])
          cpars = list(collapse(cpars))
        else:
          cpars    = np.zeros(1)
          if (vv[0] != ''): cpars[:] = float(vv[0])
      elif (vn.lower() == 'lamc0'):
        lamc0 = float(vv)
      elif (vn.lower() == 'fc'):
        fc   = float(vv)
        ...
        if (vv == 'False'):
          rnd   = False
      elif (vn.lower() == 'correlated'):
        correlated = True
        if (vv == 'False'):
          correlated = False
      elif (vn.lower() == 'inclcovar'):
        inclcovar = True
        if (vv == 'False'):
          inclcovar = False
      elif (vn.lower() == 'sigmarho'):
        sigmarho = float(vv)   
      elif (vn.lower() == 'arho'):
        arho = float(vv)   
      elif (vn.lower() == 'ntype'):
  ...
  return fnr,fnn,fns,dirout,Nlev,pmin,pmax,bg,\
         ...
         clr,fmin,mmrr,nwalkers,nstep,nburn,thin,restart,progress
#
#
...
def init(lam_lr,dlam_lr,lam_hr,species_l,species_c,opdir,pf,tf,mode=-1):
  ...
  if ( np.any( pf != -1) or np.any( tf != -1) ): # fixed p or t case
    if ( np.any( pf != -1) and np.any( tf == -1) ): # fixed p case
      ...
      p0[:] = pf
      sigma = np.exp(sigma_interp_2D(np.log10(p0),1/temp))
      sigma_interp_1D = interpolate.interp1d(1/temp,np.log(sigma),axis=1,fill_value="extrapolate")
      sigma_interp = sigma_interp_1D
    elif ( np.any( pf == -1) and np.any( tf != -1) ): # fixed t case
      t0    = np.zeros(len(press))
      t0[:] = tf
      sigma = np.exp(sigma_interp_2D(np.log10(press),1/t0))
      sigma_interp_1D = interpolate.interp1d(np.log10(press),np.log(sigma),axis=1,fill_value="extrapolate")
      sigma_interp = sigma_interp_1D
      ...
      sigma  = np.exp(sigma_interp_2D(np.log10(p0),1/temp))
      sigma_interp_1D = interpolate.interp1d(1/temp,np.log(sigma),axis=1,fill_value="extrapolate")
      sigma0 = np.exp(sigma_interp_1D(1/tf))
      def sigma_interp_0D():
        return np.log(sigma0)
...
#                 'csnr' = constant snr
#                 'cnse' = constant noise (Nicole: constant stddev)
#                 'cerr' = constant error in FpFs (Nicole: constant-size error bars)
...
#                 'corr' = noise dominated by correlated noise
#
# outputs:
#
...
def noise(lam0,snr0,lam,dlam,FpFs,Ts,ntype):
  ...
  if (ntype == 'csnr'):
    ...
    # snr = FpFs / err
    snr = snr0 * FpFs / FpFs_interp(lam0)
  elif (ntype == 'plan'):
    FpFs_interp  = interpolate.interp1d(lam,FpFs)
    res_interp   = interpolate.interp1d(lam,lam/dlam)
...
def pHG(g,cosTh):
  id      = np.copy(g)
  id[:]   = 1.
  fac     = id+np.multiply(g,g)-2*cosTh*g
  return np.divide((id-np.multiply(g,g)),fac*np.sqrt(fac))
#        np.divide((id-np.multiply(g,g)),np.power(id+np.multiply(g,g)-2*cosTh*g,1.5))
#
#
...
def pHG_int(g,cosTh):
  ...
  iz = np.where(g != 0)
  if iz[0].size !=0:
    soln[iz] = np.divide((id[iz]-np.multiply(g[iz],g[iz])),np.multiply(g[iz],np.sqrt(id[iz]+np.multiply(g[iz],g[iz])-2*g[iz])))
    soln[iz] = soln[iz] - np.divide((id[iz]-np.multiply(g[iz],g[iz])),np.multiply(g[iz],np.sqrt(id[iz]+np.multiply(g[iz],g[iz])-2*cosTh*g[iz])))  
  return soln
#
#
